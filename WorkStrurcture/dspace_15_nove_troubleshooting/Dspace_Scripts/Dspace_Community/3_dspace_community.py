...
logging.basicConfig(
    ...
    filename = datetime.now().strftime('DSPACE_Collection_15_nov_test-%d-%b-%Y.log'),
...
def remove_data():
    #try:
    conn = sql_conn("localhost", "root", "", "15_nov_dspace")
    cursor = conn.cursor()
...
#creating connection with 15_nov_dspace db
...
def file_scrapping(file_path):
    #try:
        with open(file_path, encoding='cp437'):
            ...
            col_list = ['dc.title[]','dc.identifier.uri','dc.contributor.author[]','dc.date.issued[]','dc.description.provenance[en]','dc.subject[]','dc.publisher[]']
            # col_list = ['dc.contributor.author[]','dc.date.issued[]','dc.identifier.uri','dc.description.provenance[en]','dc.title[]','dc.subject[]','dc.publisher[]']
...
def read_files(folder_path):
    for data_file in sorted(os.listdir(folder_path)):
        file_scrapping("E:/WorkStrurcture_byRaj/WorkStrurcture/dspace_15_nove_troubleshooting/Community/" + data_file)
#remove_csv()
#remove_data()
read_files("E:/WorkStrurcture_byRaj/WorkStrurcture/dspace_15_nove_troubleshooting/Community/")
