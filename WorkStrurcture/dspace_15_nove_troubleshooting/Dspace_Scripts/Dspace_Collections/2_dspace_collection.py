...
def sql_conn(hostname,username,password,dbname):
    ...
    logging.info('dspace connected successfully')
    return myconn
...
def file_scrapping(file_path):
    #try:
        with open(file_path, encoding='cp437'):
            print(file_path)
            logging.info('dspace filepath', file_path)
            col_list = ['dc.contributor.author[]','dc.date.issued[]','dc.identifier.uri','dc.description.provenance[en]','dc.title[]']
...
def read_files(folder_path):
    for data_file in sorted(os.listdir(folder_path)):
        file_scrapping("E:/WorkStrurcture_byRaj/WorkStrurcture/dspace_15_nove_troubleshooting/Collection/" + data_file)
#remove_csv()
#remove_data()
read_files("/WorkStrurcture/dspace_15_nove_troubleshooting/Collection/")
