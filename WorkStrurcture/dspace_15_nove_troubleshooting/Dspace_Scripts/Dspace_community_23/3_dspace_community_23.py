...
def file_scrapping(file_path):
    try:
        with open(file_path, encoding='cp437'):
            print(file_path)
            col_list = ['dc.contributor.author[]','dc.date.issued[]','dc.identifier.issn[]','dc.description.provenance[en]','dc.title[]','dc.subject[]','dc.publisher[]']
...
def read_files(folder_path):
    for data_file in sorted(os.listdir(folder_path)):
        file_scrapping("E:/WorkStrurcture_byRaj/WorkStrurcture/dspace/23/" + data_file)
#remove_csv()
#remove_data()
read_files("E:/WorkStrurcture_byRaj/WorkStrurcture/dspace/23/")
