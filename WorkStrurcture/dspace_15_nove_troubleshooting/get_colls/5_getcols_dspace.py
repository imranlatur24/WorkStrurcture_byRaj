...
import logging
logging.basicConfig(
    filename='C:/Users/Administrator/Documents/DRDO UPDATE/WorkStrurcture_30JUNE/WorkStrurcture/WorkStrurcture/logs//merge_dspace.log',
...
def getdata():
    ...
    for r in TITLE:
        merge_arr.append(r)
    cursor.execute("SELECT DISTINCT(publisher) from dspace")
    publisher = cursor.fetchall()
    print('publisher type', type(publisher))
    for r in publisher:
        merge_arr.append(r)
        # print(r)
...
def truncate():
    ...
    my_cur.execute('TRUNCATE TABLE `dspace_merge`')
truncate()
getdata()