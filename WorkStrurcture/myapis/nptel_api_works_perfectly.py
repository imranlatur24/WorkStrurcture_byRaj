    ...
    if re.fullmatch(pat, alphanumeric):
        ...
        if csearch:
            ...
            if types == 'ALL':
                ...
                cursor.execute("select * from libsys WHERE TITLE LIKE %s LIMIT 300", ("" + csearch + "%",))
...
def filter_wise_dspace():
    ...
    cursor = conn.cursor(pymysql.cursors.DictCursor)
    if re.fullmatch(pat, alphanumeric):
        dsearch = re.sub('[!,*)@#%(&$_?.^]', '', alphanumeric)
        dsearch = dsearch.replace('"', '')
        print('##csearch inside regrex block')
    try:
        if dsearch:
            cursor.execute(
                "SELECT * FROM `dspace` WHERE TITLE LIKE\"%" + dsearch + "%\" AND author LIKE \"%" + author + "%\" AND subject LIKE \"%" + subject + "%\" AND date_issued LIKE\"%" + date_issued + "%\"")
...
def get_autocomplete_all():
    ...
    try:
        ...
        cursor.execute("select DISTINCT(TITLE) from dspace where TITLE LIKE\"" + csearch + "%\"ORDER BY TITLE")
        wp_comments_janes = cursor.fetchall()
        for jrow in wp_comments_janes:
            nptel.append(jrow)
...
def filter_wise_janes():
    ...
    pat = re.compile(r"[A-Za-z0-9]+")
    if re.fullmatch(pat, alphanumeric):
        jsearch = re.sub('[!,*)@#%(&$_?.^]', '', alphanumeric)
        jsearch = jsearch.replace('"', '')
