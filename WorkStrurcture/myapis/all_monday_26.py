        ...
        if re.fullmatch(pat, alphanumeric):
            ...
            if csearch:
                ...
                if types == 'ALL':
                    ...
                    # (SELECT * FROM `hand` WHERE title LIKE'%ammunition%' LIMIT 300) UNION ALL (SELECT * FROM `hand`WHERE company LIKE '%ammunition%' LIMIT 300) UNION ALL (SELECT * FROM `hand`WHERE section LIKE'%ammunition%' LIMIT 300) UNION ALL (SELECT * FROM hand WHERE general_title LIKE'%ammunition%' LIMIT 300);
                    cursor.execute(
                        "(select * from hand where TITLE LIKE\"%" + csearch + "%\" LIMIT 300) UNION ALL (select * from hand where company LIKE \"%" + csearch + "%\" LIMIT 300) UNION ALL (select * from hand where section LIKE \"%" + csearch + "%\" LIMIT 300) UNION ALL (select * from hand where general_title LIKE \"%" + csearch + "%\" LIMIT 300)")
                    logging.info('Janes Handbook SQL Query executing')
                    hand_janes = cursor.fetchall()
...
def filter_wise_janes():
    ...
    if re.fullmatch(pat, alphanumeric):
        ...
        testing_rows = cursor.fetchall()
        print('total count ', len(testing_rows))
        print('testing_rows inside else block ', testing_rows)
        for row in testing_rows:
            janes.append(row)
            status_db.append(row['status'])
            country_db.append(row['country'])
