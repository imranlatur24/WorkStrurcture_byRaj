...
def get_user_register():
    # try:
    #working for android side
    if request.method == 'POST' and 'first_name' in request.values and 'last_name' in request.values and 'empid' in request.values and 'sci_type' in request.values and 'password' in request.values and 'cpassword' in request.values:
    ...
    conn = mysql.connect()
    cursor = conn.cursor(pymysql.cursors.DictCursor)
    sql = 'INSERT INTO `test`(`id`, `first_name`, `last_name`, `empid`, `sci_type`, `password`, `cpassword`) values(NULL, %s, %s,%s, %s,%s, %s)'
    values = (first_name, last_name, EmpID, sci_type, PASSWORD, CPASSWORD)
    cursor.execute(sql, values)
    conn.commit()
