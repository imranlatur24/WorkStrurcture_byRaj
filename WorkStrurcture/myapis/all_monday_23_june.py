...
date_issued_db = []
logging.basicConfig(
    filename='C:/Users/3029/PycharmProjects/WorkStrurcture/WorkStrurcture/logs/apis_report.log',
...
def get_user():
    ...
    # creation of empty arraylist
    alphanumeric = request.args.get('q')
    ...
    if re.fullmatch(pat, alphanumeric):
        csearch = re.sub('[!,*)@#%(&$_?.^]', '', alphanumeric)
        csearch = csearch.replace('"', '')
        print('##csearch inside regrex block')
        print('csearch in regrex block ',csearch)
        if csearch:
            conn = mysql.connect()
            cursor = conn.cursor(pymysql.cursors.DictCursor)
            if types == 'ALL':
                cursor.execute(#"SELECT * FROM `child_real` WHERE TITLE LIKE\"%" + csearch + "%\"")
                    "(select * from child_real where TITLE LIKE\"" + csearch + "%\" LIMIT 300) UNION ALL (select * from child_real where coordinators LIKE \"" + csearch + "%\" LIMIT 300) UNION ALL (select * from child_real where Institute LIKE \"" + csearch + "%\" LIMIT 300) UNION ALL (select * from child_real where disciplineName LIKE \"" + csearch + "%\" LIMIT 300) UNION ALL (select * from child_real where subjectName LIKE \"" + csearch + "%\" LIMIT 300)")
                ...
                cursor.execute(#"SELECT * FROM `hand` WHERE TITLE LIKE\"%" + csearch + "%\"")
                    "(select * from hand where TITLE LIKE\"" + csearch + "%\" LIMIT 300) UNION ALL (select * from hand where company LIKE \"" + csearch + "%\" LIMIT 300) UNION ALL (select * from hand where section LIKE \"" + csearch + "%\" LIMIT 300) UNION ALL (select * from hand where general_title LIKE \"" + csearch + "%\" LIMIT 300)")
                logging.info('Janes Handbook SQL Query executing')
                hand_janes = cursor.fetchall()
                for jrow in hand_janes:
                    common_db.append(jrow)
                    logging.info(
                        'appending James Handbook data to janes Array', jrow)
                cursor.execute(
                    "select * from libsys WHERE TITLE LIKE %s LIMIT 300", ("%" + csearch + "%",))
                ...
                cursor.execute(
                    "select * from armablog_filepath WHERE TITLE LIKE %s LIMIT 300", ("%" +csearch + "%",))
                ...
                cursor.execute(
                    "(select * from dspace where TITLE LIKE\"" + csearch + "%\" LIMIT 300) UNION ALL (select * from dspace where author LIKE\"" + csearch + "%\" LIMIT 300) UNION ALL (select * from dspace where DESCRIPTION LIKE\"" + csearch + "%\" LIMIT 300) UNION ALL (select * from dspace where subject LIKE\"" + csearch + "%\" LIMIT 300)")
                ...
                for jrow in hand_janes:
                    #print('jrow', jrow)
                    common_db.append(jrow)
                    # print('janes db = ',common_db['status'])
                    #print('janes db = ', common_db)
                    for db in common_db:
                        #print('db in janes common ', db)
                        status_db.append(db['status'])
                        country_db.append(db['country'])
                        section_db.append(db['section'])
                        general_title_db.append(db['general_title'])
            ...
            return resp
    else:#work for qouatation here best match

        csearch = alphanumeric
        csearch = csearch.replace('"', '')
        csearch = csearch.replace("'", '')
        csearch = re.sub(r'[?|[#|@|$|.|!]', r'', csearch)
        print('csearch in else block ', csearch)
        print('##csearch inside else block')
        if csearch:
            conn = mysql.connect()
            cursor = conn.cursor(pymysql.cursors.DictCursor)
            ...
            if types == 'ALL':
                ...
                #cursor.execute("SELECT * FROM `dspace` WHERE TITLE LIKE\"" + csearch + "%\"")
                cursor.execute(
                    "(select * from dspace where TITLE LIKE\"%" + csearch + "%\" LIMIT 300) UNION ALL (select * from dspace where author LIKE \"%" + csearch + "%\" LIMIT 300) UNION ALL (select * from dspace where DESCRIPTION LIKE \"%" + csearch + "%\" LIMIT 300) UNION ALL (select * from dspace where subject LIKE \"%" + csearch + "%\" LIMIT 300)")
                logging.info('dspace SQL Query executing')
                metadatavalue_janes = cursor.fetchall()
                ...
                '''cursor.execute(
                    "select * from libsys WHERE TITLE LIKE %s LIMIT 150", ("%" + csearch + "%",))'''
                # logging.info('LIBSYS SQL Query executing')
                lib = cursor.fetchall()
                for jrow in lib:
                    common_db.append(jrow)
...
def filter_wise_dspace():
    ...
    alphanumeric = data['dsearch']
    author = data['author']
    subject = data['subject']
    date_issued = data['date_issued']
    print('dsearch', alphanumeric)
    print('author ', author)
    ...
    try:
        ...
        print(e,'exception inside filter_wise_dspace api')
    finally:
        cursor.close()
        conn.close()
    ...
    try:
        ...
        # DSPACE
            ...
            for jrow in wp_comments_nptel:
                nptel.append(jrow)
            combine_arr = {**mydictionary_nptel}
            print("result of dspace data : ", nptel)
            resp = jsonify(combine_arr)
            resp.status_code = 200
            return resp
...
def get_autocomplete_view1():
    ...
    try:
        ...
        conn = mysql.connect()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute("select DISTINCT(TITLE) from all_merge where TITLE LIKE\"%" + csearch + "\"")
...
@app.route('/autocompleteview_nptl', methods=['POST', 'GET'])
def get_autocomplete_view2():
    ...
    try:
        ...
        conn = mysql.connect()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute("select * from nptel_merge where TITLE like '%"+csearch+"%'")
        wp_comments_janes = cursor.fetchall()
        for jrow in wp_comments_janes:
            nptel.append(jrow)
...
def get_user_login():
    ...
    for row in result:
        print(row)
        # print(row[7])
        name_match = int(row['EmpID'])
        print('name matched ', name_match)
        password_match = row['PASSWORD']
        print('pass matched ', password_match)
        #resp = jsonify(message="user logged in successful ")
        # return resp
        print('login EmpID',type(EmpID))
        print('login name_match',name_match)
        if EmpID == name_match:
            resp = jsonify(message="logged in successful ", data=row)
            resp.status_code = 200
            return resp
        elif EmpID != name_match:
            resp = jsonify(
...
def get_notification_count():
    ...
    try:
        ...
        #
        for row in result:
            row['COUNT'] = row.pop('count( DISTINCT(search_hst) )')
            print('row child', row)
            resp = row
            return resp
    except Exception as e:
...
def filter_wise_nptel():
    ...
    if re.fullmatch(pat, alphanumeric):
        csearch = re.sub('[!,*)@#%(&$_?.^]', '', alphanumeric)
        csearch = csearch.replace('"', '')
        print('##csearch inside regres block in nptel')
        if csearch:
            cursor.execute(
        ...
        csearch = csearch.replace('"', '')
        csearch = csearch.replace("'", '')
        print('##csearch inside else block in nptel')
        if csearch:
            cursor.execute(
                "SELECT * FROM `child_real` WHERE TITLE LIKE\"%" + csearch + "%\" AND disciplineName LIKE \"%" + disciplineName + "%\" AND subjectName LIKE \"%" + subjectName + "%\" AND Institute LIKE\"%" + institute + "%\" AND filetype LIKE \"%" + filetype + "%\" AND coordinators LIKE \"%" + coordinator + "%\"")
            testing_rows = cursor.fetchall()
...
def filter_wise_janes():
    ...
    if re.fullmatch(pat, alphanumeric):
        csearch = re.sub('[!,*)@#%(&$_?.^]', '', alphanumeric)
        csearch = csearch.replace('"', '')
        print('##csearch inside regrex block inside janes')
        if csearch:
            cursor.execute(
                "select * from hand where TITLE LIKE\"%" + csearch + "%\" AND country LIKE \"%" + country + "%\" AND section LIKE \"%" + section + "%\" AND general_title LIKE\"%" + general_title + "%\" AND status LIKE \"%" + status + "%\"")
            testing_rows = cursor.fetchall()
        ...
        csearch = csearch.replace('"', '')
        csearch = csearch.replace("'", '')
        print('##csearch inside else block inside janes')
        if csearch:
            cursor.execute(
                "select * from hand where TITLE LIKE\"%" + csearch + "%\" AND country LIKE \"%" + country + "%\" AND section LIKE \"%" + section + "%\" AND general_title LIKE\"%" + general_title + "%\" AND status LIKE \"%" + status + "%\"")
            testing_rows = cursor.fetchall()
